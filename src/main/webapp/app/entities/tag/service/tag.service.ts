...
export class TagService {
  ...
  update(tag: ITag): Observable<EntityResponseType> {
    return this.http.put<ITag>(`${this.resourceUrl}/${getTagIdentifier(tag) as string}`, tag, { observe: 'response' });
  }

  partialUpdate(tag: ITag): Observable<EntityResponseType> {
    return this.http.patch<ITag>(`${this.resourceUrl}/${getTagIdentifier(tag) as string}`, tag, { observe: 'response' });
  }
