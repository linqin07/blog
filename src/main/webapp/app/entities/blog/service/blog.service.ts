...
export class BlogService {
  ...
  update(blog: IBlog): Observable<EntityResponseType> {
    return this.http.put<IBlog>(`${this.resourceUrl}/${getBlogIdentifier(blog) as string}`, blog, { observe: 'response' });
  }

  partialUpdate(blog: IBlog): Observable<EntityResponseType> {
    return this.http.patch<IBlog>(`${this.resourceUrl}/${getBlogIdentifier(blog) as string}`, blog, { observe: 'response' });
  }
